...
export default function PhotosUploader({ addedPhotos, setAddedPhoto }) {
    ...
    function removeLink(ev, filename) {
        ev.preventDefault();
        let temp = addedPhotos.filter(photo => (photo !== filename));
            ...
            <div className='mt-2 grid grid-cols-3 gap-2 md:grid-cols-4 lg:grid-cols-6'>
                {addedPhotos.length > 0 && addedPhotos.map((photolink) => (
                    <div className='h-32 flex relative' key={photolink}>
                        ...
                        <button onClick={(ev) => removeLink(ev,photolink)} className="absolute bottom-0 right-0 bg-gray-200 bg-opacity-50 p-1 rounded-2xl">
                        ...
                        <button onClick={ev => {selectAsMainPhoto(ev,photolink)}} className="absolute bottom-1 left-1 bg-gray-200 bg-opacity-50 rounded-2xl p-1">
